...
- **`start`** (required) — argv that starts the unit and exits (e.g.
  `docker run -d ...`). A non-zero exit marks the launch as crashed. Its
  stderr (and `reset`'s) is written to the node's run log, so a failed
  start's error shows up in `sheppy logs`.
