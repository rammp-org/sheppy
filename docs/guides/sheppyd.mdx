...
> **Scope.** Today `sheppyd` is *local only* — it supervises processes on the
> machine it runs on and ignores the `machine:` field on alternatives.
> Multi-machine supervision over SSH is on the [roadmap](../../architecture#roadmap).
...
|---------|--------------|
| `sheppy up <profile> [--manifest PATH]` | resolve the profile, print the plan, converge the running system to it, wait until it settles, print final states. Exits non-zero if any node crashed. `--manifest` defaults to `sheppy-manifest.yaml`. |
...
| `sheppy --version` (`-V`) | print the installed version and exit |
...
When a node has a `Δ`, the PROCESS tab adds a `drift` line under its state
saying why:
...
| `Δ params differ: <names>` | the running process was launched with different values for these parameters |
| `Δ launch command changed since start` | the selection now resolves to a different command than the one running, usually because the manifest was edited after launch |
| `Δ running <alt>, not in this manifest` | the running alternative is no longer listed under this node |

`space`/`L` push what you selected onto what's running; `!` pulls it back. See
[desired vs actual](../../concepts#desired-vs-actual).
...
| `op` | Effect |
|------|--------|
| `launch` | start `spec.descriptor`; replaces any different process already running for that node (one process per node) |
