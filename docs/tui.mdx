...
| `Shift+L` | Apply the whole selection: shows a plan of starts, restarts, and stops, then asks to confirm. Nodes from other manifests are left alone |
...
`p` don't apply, and `Shift+L` leaves them alone. `Shift+X` and `sheppy down` stop them
