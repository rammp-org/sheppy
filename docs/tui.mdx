...
```
 🐑 sheppy │ ◆ profile <none>* │ examples/local-demo.yaml · 2 nodes · ● 1 running          ✓ no errors  │ ◷ 18:07:49
 MACHINES  — none declared — · …
       NODE       ALTERNATIVE   HOST    USAGE     │  ALTERNATIVES · worker · single      │DETAIL TOPICS PROCESS YAML
█ ●    clock      ticker        —       0% 15M    │█ ○ steady                            │
█ ✕ Δ  worker     flaky         —       0% 15M    │█ process · python3 -c 'import time;  │  state       ✕ crashed
                                                  │█                                     │  drift       Δ flaky selected, not running
                                                  │█ ◉ flaky                             │  pid         3948421
                                                  │█ process · python3 -c 'import time;  │  exit code   1
  ↑↓  move   ⏎  select   s  save   l  load   p  params   e  errors   1-4  tabs   ␣  apply   x  stop   r  woof   L  converge   !  snap
```
...
- **Footer:** key hints, and whether `sheppyd` is running.
...
| `Enter` | On a node: move into its alternatives. On an alternative: select it |
| `Esc` | Back to the node list |
| `1`–`4` | Switch detail tab: DETAIL, TOPICS, PROCESS, YAML |
| `space` | Apply the selection to the highlighted node: launch the selected alternative (replacing whatever that node is running), or stop the node if nothing is selected |
| `x` | Stop the highlighted node |
| `r` | Restart the highlighted node with the command it was launched with. To apply a manifest or param edit, use `space` |
| `L` | Apply the whole selection: shows a plan of starts, restarts, and stops, then asks to confirm. Nodes from other manifests are left alone |
| `X` | Stop everything `sheppyd` is running, including nodes from other manifests (asks to confirm) |
| `!` | Snapshot: set the selection to what's running. Press `s` to save it |
...
| `Ctrl+Q` | Quit. Running nodes keep running |
...
`p` don't apply, and `L` leaves them alone. `X` and `sheppy down` stop them
along with everything else.

## Opening without a profile

The TUI always opens with no profile loaded (`profile <none>` in the header).
If `sheppyd` is already supervising nodes when the TUI connects and nothing is
selected yet, the TUI takes what's running as the starting selection,
including each node's parameter overrides. A system you brought up earlier,
for example with `sheppy up`, doesn't show as drift.

- Running, launching, and crashed nodes are adopted. A crashed node keeps its
  `Δ` (selected, not running), and `space` relaunches it.
- Stopped nodes are not adopted.
- Nodes that aren't in this manifest stay under the [orphans](#orphans)
  divider.
- Selections you made before `sheppyd` was running are kept.
- Nodes started from elsewhere after the TUI connects show `Δ` as usual.

The header still reads `profile <none>` with no `*`; press `s` to save the
adopted selection as a profile.
