...
daemon launched. The file carries `"format": 1`; a daemon that finds a format
it does not know adopts nothing, and the children keep running unsupervised.
...
**crashed** rather than flickering through *running*. A command that cannot be
started at all (not found, not executable) is reported as **crashed** too, with
the reason as the only line in that run's log.
...
A stop is not finished when the direct child exits; it is finished when the
whole process group is empty. If the child dies on `SIGINT` but a helper it
spawned (a camera pipeline, a serial bridge) ignores the signal and lives on,
the escalation continues against the group — `SIGTERM`, then `SIGKILL` —
until nothing in it remains or the grace budget is spent. A stop, restart,
or relaunch of a running node waits for this. If the leader crashed on its
own, see #163.
...
keys, and keys whose value has the wrong type (`"stop_grace": "5"`), are
ignored with a warning in the daemon log and the default is used. A missing or
malformed file falls back to defaults. `log_dir` expands a leading `~`.
...
```json
{"event": "hello", "sheppyd": "1.0.0", "protocol": 2}
```

`sheppyd` is the installed package version, `protocol` the wire format
number. The CLI and TUI compare `sheppyd` with their own version and warn
when they differ, which happens after an upgrade while the old daemon is
still running; `sheppy daemon stop` ends it, and the next command starts
one on the new code.
...
| `logs` | return the last `n` ring-buffer lines for a node; `n` must be at least 1 (0 or less returns none) |
