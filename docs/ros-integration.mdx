...
| The manifest | quit and reopen the TUI, then `space`; or `sheppy up <profile>` | the TUI reads the manifest only when it opens, and `r` and `woof` rerun the command stored at launch |
