...
2. **Profiles instead of launch-file sprawl.** Launch files that switch
   ...
   and `sheppy up <profile>` brings exactly that system back.
3. **See and tune every node.** The TUI shows each node's state, CPU and
   memory, and latest output, flags where what's running differs from what you
   selected, and lets you change a node's parameters and relaunch it.
4. **More than `ros2 launch`.** Alternatives can be ROS executables and launch
   files, Docker containers, or any command, like a simulator GUI, all
   supervised the same way. They keep running after you close the TUI.
...
- **[Daemon](./concepts#what-happens-when-you-press-space)** — `sheppyd`, which actually launches and supervises processes.
