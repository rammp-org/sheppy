...
alternative (mock vs. real) per node. Save and load those choices — plus
per-alternative parameter overrides — as named **profiles**. Select, press
`space`, and `sheppyd` really runs it — locally, surviving TUI detach.
Multi-machine launch and live graph introspection are planned; see the
