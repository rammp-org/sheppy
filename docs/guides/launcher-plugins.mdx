...
This guide describes the code in `sheppy/launch/base.py`,
...
small, stdlib-only supervisor that must stay simple, because it keeps your
nodes running after the TUI exits. If launcher code ran
...
plugin only needs to be importable by the TUI and CLI; `sheppyd` never
imports it.
...
- **`launch(alt, params, ctx)`** — called at resolve time
  ...
  the built `Alternative` (has `.id`, `.kind`, `.machine`, and `.raw` — the
  full raw dict for this alternative, so any custom keys your `kind` needs
  are always available via `alt.raw`, even if they aren't among the
  built-in typed fields like `.package`/`.executable`/`.command`; `.config`
  is a deprecated alias for `.raw` that will be removed after 1.0). `params`
...
- **`start`** (required) — argv that starts the unit and exits (e.g.
  `docker run -d ...`). The node shows as *launching* from the moment
  `start` begins (a `docker run` may pull for minutes) and is recorded in
  the daemon's state file, so a daemon restart mid-start re-adopts the unit
  if it has come up; if it has not, the node shows crashed and the next
  launch's `reset` cleans up whatever the orphaned start produced (see
  #165). A non-zero exit marks the launch as crashed. Its stderr (and
  `reset`'s) is written to the node's run log, so a failed start's error
  shows up in `sheppy logs`.
...
- **`stop`** (optional) — argv to gracefully stop the unit (e.g.
  ...
  intervention. If it exits non-zero, or the unit is still up
  `stop_grace + kill_grace` seconds later, the daemon stops waiting, reaps
  its `watch`/`logs` companions and marks the node stopped even though the
  unit may still be running; the next launch's `reset` is what cleans it up.
...
the example test does. `resolve()` also runs it on whatever `launch()`
returns: a value that is not a `LaunchDescriptor`, or one that fails
`validate()`, is reported as a warning (`'<node>': launcher '<kind>'
returned ...`) and the node is not launched, the same path an exception from
`launch()` takes. The daemon independently re-checks the same
...
The key (`echo`) is cosmetic (the entry point *name*); what selects
...
metadata picks them up. From a source checkout:

```bash
uv sync
```

If sheppy was installed with the one-line installer, add your plugin to that
tool environment instead:

```bash
uv tool install --force --with <your-plugin> git+https://github.com/rammp-org/sheppy@<ref>
```

with `<ref>` the release you run (`sheppy --version`, so `v1.0.0`) or `dev`.
...
Each `kind` can be registered once. If two plugins claim the same kind, the
first one discovered keeps it and the other is ignored with a warning on
stderr — `sheppy: ignoring launcher <Class> for kind '<kind>': already
registered by <Class>`. Entry-point order is undefined, so a plugin cannot
rely on replacing a built-in kind; pick a new kind name instead.
...
> **Not runnable as shipped.** Nothing registers `echo_launcher.py` under
> `sheppy.launchers`, so a manifest using `kind: echo` fails validation with
> an unknown-kind error until you package the file and register it as shown
> above. It is a reference to read and test, not a plugin you can use as-is.

- `validate({"kind": "echo", "message": "hello ..."})` checks the one field
  this `kind` requires and returns `[]` (valid) or an error string.
- `launch()` reads `alt.raw["message"]` — `.raw` is the full raw
  ...
  `inherit` descriptor. The message goes through `shlex.quote` before it is
  spliced into the `bash -c` string, so a `'` or `$HOME` in the manifest is
  echoed literally instead of being parsed by the shell; do the same for any
  manifest text your launcher puts in a command line. The `sleep 3600` keeps the process alive after the
