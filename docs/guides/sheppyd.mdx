...
until nothing in it remains or the grace budget is spent. A `restart` or
relaunch of that node waits for this, so a new instance never fights a
leftover helper for the hardware.
