---
title: Running with ROS 2
---

# Running with ROS 2

This page covers what sheppy does with your ROS environment and what it
leaves to you. Read it before pointing sheppy at a real workspace.
...
- **Changing your shell later changes nothing.** Re-sourcing, or exporting a
  different `ROS_DOMAIN_ID` in another terminal, doesn't reach a `sheppyd`
  that is already running. To switch, stop everything and start again from a
  terminal that has the new environment:
  ...
  `sheppy daemon stop` restarts only the daemon. Nodes that are already
  running keep their old environment until you relaunch them.
...
- It applies only to alternatives that set `machine:`. There is no default
  machine.
- Use an absolute path. `~` is not expanded, so `~/ws/install/setup.bash`
  fails.
...
manifest's. Use absolute paths. A `compose:` file path is the exception: it is
...
|-------------|------|-----|
| Rebuilt a package (`colcon build`) | `r` in the TUI, or `sheppy woof <node>` | a restart reruns the same command, which picks up the new build |
| The manifest, or a param | `space` in the TUI, or `sheppy up <profile>` | these rebuild the command; `r` and `woof` rerun the command stored at launch |
...
them against what is actually on the wire. Use `ros2 topic info -v` for that.
