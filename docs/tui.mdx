...
| `Enter` | On a node: move into its alternatives. On an alternative: select it, or de-select it if it's already selected, which leaves the node with nothing to launch and drops its param overrides |
